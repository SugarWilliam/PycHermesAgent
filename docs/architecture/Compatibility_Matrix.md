...
**GitHub Actions (Phase 4 engineering tier):** `contract-tests` (Python matrix + MRAG proofs + simulated packaging probe + `scripts/release_gates.py` with ruff+mypy), `production-gates` (`RELEASE_GATES_PRODUCTION=1` SBOM MRAG migrate + Linux `npm run dist:dir` + `electron_dist_layout_smoke --prefer-unpacked linux`), and `desktop-windows-unpacked` (Windows **`win-unpacked`** + same smoke with `--prefer-unpacked win`). This does **not** replace code signing / store-ready attestation (`Production_Release_Gates.md` Honest scope).
