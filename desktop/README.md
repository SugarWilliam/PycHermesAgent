...
- **Knowledge bases:** `GET /knowledge-bases` lists local MRAG KBs (`listKnowledgeBases()`). `POST /knowledge-bases/{id}/search` runs retrieval (`searchKnowledgeBase()`). **`POST /knowledge-bases/{id}/materialize`** ingests finalized text with server-side bookkeeping metadata (`materializeKnowledgeBaseText()`). Paths mirror **`SIDECAR_PATHS`** / **`sidecarKnowledgeBasePath()`** in `sidecarClient.js`.
- **Skill patch drafts (weak automation):** `GET /skills/patch-drafts` (`listSkillPatchDrafts()`), **`POST`** create (`createSkillPatchDraft`), **`POST`** `.../suggest-from-session` (`suggestSkillPatchDraftFromSession`), **`DELETE .../{draft_id}`** (`deleteSkillPatchDraft`) — drafts are human-gated only; **`sidecarSkillPatchDraftPath`** builds the DELETE path segment.
